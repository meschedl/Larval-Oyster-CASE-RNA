...
are these the files I want?
```{r}
CASE_SIG <- read.delim("ordered_sig_CASE.txt")
SE_SIG <- read.delim("ordered_sig_CA.txt")
CA_SIG <- read.delim("ordered_sig_SE.txt")
```

running from DESeq R script for the results files actually, these I want because I can do the subset function on them 
```{r}
...
Up_Sig_SE <- subset(sig_SE, log2FoldChange>0)
Down_Sig_SE <- subset(sig_SE, log2FoldChange<0)
